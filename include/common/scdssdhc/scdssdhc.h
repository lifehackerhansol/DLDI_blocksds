...
#define SCDSSDHC_CTRL_BASE             (MCCNT1_ENABLE | MCCNT1_RESET_OFF | MCCNT1_LATENCY2(24) | MCCNT1_LATENCY1(0))
...
static inline u64 SCDSSDHC_CMD_SRAM_READ_DATA(u32 address)
{
	return (0x70ull << 56) | ((u64)address << 24);
}

static inline u64 SCDSSDHC_CMD_SRAM_WRITE_DATA(u32 address)
{
...
/*
	...
	return non-0 == SDHC
*/
static inline u64 SCDSSDHC_CMD_SD_IS_SDHC(void)
{
	return SCDSSDHC_CMD_SRAM_READ_DATA(0x7F9E0);
}

// FIFO commands
#define SCDSSDHC_CMD_FIFO_BUSY        (0x80ull << 56)
#define SCDSSDHC_CMD_FIFO_READ_DATA   (0x81ull << 56)
#define SCDSSDHC_CMD_FIFO_WRITE_DATA  (0x82ull << 56)
...
enum SCDSSDHC_SD_HOST_MODES {
	SCDSSDHC_SD_HOST_NORESPONSE = 0ull,
	SCDSSDHC_SD_HOST_READ_4B = 1ull,
	SCDSSDHC_SD_HOST_READ_4B_MULTI = 2ull, // use mode 3 to continue this read
	SCDSSDHC_SD_HOST_NEXT_4B = 3ull,
	SCDSSDHC_SD_HOST_SEND_CLK = 4ull,
	SCDSSDHC_SD_HOST_SEND_STOP_CLK = 5ull,
	SCDSSDHC_SD_HOST_READ_DATABLOCK = 6ull,
	SCDSSDHC_SD_HOST_NEXT_DATABLOCK = 7ull,
	SCDSSDHC_SD_HOST_CMD17_READ_DATA = 8ull, // Send SDIO CMD17 & read data
	SCDSSDHC_SD_HOST_CMD18_READ_DATA = 9ull, // Send SDIO CMD18 & read data until stop
	SCDSSDHC_SD_HOST_COMMIT_FIFO_DATA = 0xAull, // commit data in FIFO to SD card
	SCDSSDHC_SD_HOST_CMD24_WRITE_DATA = 0xBull, // Send SDIO CMD24 & send data in SRAM buffer
	SCDSSDHC_SD_HOST_WAIT_BUSY = 0xCull // wait until data transfer ends
};
...
*/
static inline u64 SCDSSDHC_CMD_SD_HOST_PARAM(u8 sdio, u32 parameter, u8 mode)
{
...
#define SCDSSDHC_CMD_SD_HOST_BUSY     (0x50ull << 56)

// Gets response of SD_HOST commands, if the sent mode is 1 or 2
#define SCDSSDHC_CMD_SD_HOST_RESPONSE (0x52ull << 56)
// Stops SD host data transfer
#define SCDSSDHC_CMD_SD_WRITE_END     (0x56ull << 56)

// Sends CMD17
// This is effectively 0x51/0x50/0x52 rolled into one command
static inline u64 SCDSSDHC_CMD_SD_READ_SINGLE_BLOCK(u32 sector)
{
...
// Sends CMD18
// This is effectively 0x51/0x50/0x52 rolled into one command
static inline u64 SCDSSDHC_CMD_SD_READ_MULTI_BLOCK(u32 sector)
{
...
static inline u64 SCDSSDHC_CMD_SD_HOST_SET_REGISTER(u8 bits)
{
