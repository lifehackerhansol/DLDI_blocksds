...
{
	u64 command = ((u64)SCDSSDHC_CMD_SD_HOST_PARAM << 56) | ((u64)parameter << 24) | ((u64)sdio << 16) | ((u64)response_type << 8);
	card_romSetCmd(command);
	card_romStartXfer(SCDSSDHC_CTRL_READ_4B | MCCNT1_LATENCY1(latency), false);
...
bool SCDSSDHC_SDInitialize(void)
{
	...
	// Init
	SCDSSDHC_SDHostSetRegister(SCDSSDHC_SD_HOST_REG_RESET | SCDSSDHC_SD_HOST_REG_400KHZ_CLK | SCDSSDHC_SD_HOST_REG_CLEAN_ROM_MODE);
	...
	if(isSDHC)
		SCDSSDHC_SDHostSetRegister(SCDSSDHC_SD_HOST_REG_RESET | SCDSSDHC_SD_HOST_REG_CLEAN_ROM_MODE | SCDSSDHC_SD_HOST_REG_SDHC);
...
{
	u64 command = ((u64)SCDSSDHC_CMD_SD_READ_SINGLE_BLOCK << 56) | ((u64)(isSDHC ? sector : sector << 9) << 24);
	card_romSetCmd(command);
	card_romStartXfer(SCDSSDHC_CTRL_READ_4B, false);
	card_romWaitDataReady();
	card_romGetData();
...
{
	u64 command = ((u64)SCDSSDHC_CMD_SD_READ_MULTI_BLOCK << 56) | ((u64)(isSDHC ? sector : sector << 9) << 24);
	card_romSetCmd(command);
	card_romStartXfer(SCDSSDHC_CTRL_READ_4B, false);
	card_romWaitDataReady();
	card_romGetData();
