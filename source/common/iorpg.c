...
static void ioRPG_SDWaitForState(uint8_t state) {
	...
	do {
		data = cardExt_ReadData4Byte(IORPG_CMD_SD_READ_STATE, IORPG_CTRL_READ_4B | MCCNT1_LATENCY1(4)) & mask;
	} while(data != state);
}
...
void ioRPG_SDReadSingleSector(u32 sector, void* buffer)
{
	...
	cardExt_ReadData(IORPG_CMD_CARD_READ_DATA, (IORPG_CTRL_READ_512B | MCCNT1_LATENCY1(4)), buffer, 128);
